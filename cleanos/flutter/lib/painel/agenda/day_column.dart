...
class _BlocoOSState extends State<_BlocoOS> {
  ...
  Widget build(BuildContext context) {
    ...
    final os = widget.os;
    final faixa = faixaHoraria(p.startMin, p.duracaoMin);
    // Só blocos bem curtos (<30min) colapsam em 1 linha — a partir de 30min
            ...
            child: Padding(
              ...
              child: LayoutBuilder(
                builder: (context, constraints) {
                  ...
                  // Em bloco de 1h (~52px úteis) cabem ~4 linhas → horário +
                  // cliente + serviço + valor·bairro.
                  final linhasCabem =
                      (constraints.maxHeight / 11).floor().clamp(1, 6);
